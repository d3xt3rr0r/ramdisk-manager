...
def mount_ramdisk(size_gb):
    ...
    try:
        ...
        print("Błąd montowania RAMdisk")

def remove_ramdisk():
    if os.path.exists(RAMDISK_PATH):
        try:
            subprocess.run(["sudo", "umount", RAMDISK_PATH], check=True)
        except subprocess.CalledProcessError:
            subprocess.run(["sudo", "umount", "-l", RAMDISK_PATH])
...
def start_ramdisk(size_gb=None):
    ...
    # Jeśli podano size w CLI -> użyj go
